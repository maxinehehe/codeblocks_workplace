...
void ShortestPath_Dijkstra(MGraph *G,int v0,int p[][Max_value],int D[]){
    int v,w,i,j,min;//定义起点
    bool final[50];

    for(v=0;v<G->vexnum;v++){
        final[v]=false;//设置初始值
        D[v]=G->arcs[v0][v].adj;////D[]存放v0到v得最短距离，初值为v0到v的直接距离
    ...
    D[v0]=0;//v0到v0距离为0
    final[v0]=true;//v0顶点并入集

    for(i=1;i<G->vexnum;i++){//其余G.vexnum-1个顶点
        //开始主循环，每次求得v0到某个顶点v的最短路径，并将v并入S集，然后更新p和D
        min = INFINITY;
        for(w=0;w<G->vexnum;w++){//对所有顶点检查
            if(!final[w] && D[w]<min)//在S集之外(即final[]=false)的
            //顶点中找离v0最近的顶点，将其赋给v,距离赋给min
            {
                v = w;
                min = D[w];
            }
...
void Dij_Get_shortPath(MGraph *G){
    int i,j,a,k;
    int p[Max_value][Max_value];//最短路径数组
    int D[Max_value];//最短距离数组
    City_List(G);
    printf("请输入城市序号：");
    scanf("%d",&a);
    ShortestPath_Dijkstra(G,a,p,D);
    printf("%d.%s到各城市间最短路径及长度为：\n",a,G->Vexs[a]);
    //printf("最短路径长度\t最短路径\n");
    for(i=0;i<G->vexnum;i++){
        if(i!=0 && D[i]!=INFINITY){
            printf("%s - %s的最短路径长度为：%d ",G->Vexs[a],G->Vexs[i],D[i]);
            printf("   最短路径为：%s",G->Vexs[a]);
            for(j=1;j<G->vexnum;j++){
                if(p[i][j]>-1)
...
{
    int v,w,k,min;
    int final[Max_value];                    /* final[w]=1表示求得顶点v0至vw的最短路径 */
